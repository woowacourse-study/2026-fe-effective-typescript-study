...
- 이름을 지을 때는 포함된 내용이나 계산 방식이 아니라 데이터 자체가 무엇인지 고려해야 한다.

## 아이템 42

프로젝트 외부에서 비롯된 타입은 직접 타입 선언을 작성하지말고, 타입을 import하거나 명세로부터 자동생성하는 것이 낫다.

-> 직접 타입을 작성하게 되면 엣지 케이스들을 고려하지 못할 가능성이 높기 때문이다.

명세 정보나 공식 타입을 얻어서 사용하자. 만약 없다면 quicktype 같은 도구를 사용해서 타입을 자동생성하자.
