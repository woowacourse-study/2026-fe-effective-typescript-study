...
## 아이템 39 타입 간 차이점을 모델링하기보다 같은 타입으로 통일하기

타입 변환 도구들이 탑재되어 있고, 이를 사용하면 생산성이 향상됨
DB 칼럼은 `first_name`처럼 snake_case이다. 하지만 TS에서는 보통 카멜 케이스를 쓴다.

ObjectToCamel<T>과 같이 직접 변환 메서드를 만들 수도 있다.

꼭 통일시켜야 하는 것은 아니다. snake_case를 TS에서 써도 되지만 그 불편함은 감수해야 한다.

사실 이렇게 차이가 있는 타입을 모델링하기보다는 애초에 타입을 통일시키는게 낫다. 하지만 DB, API가 이렇게 고정되어있다면 변환 헬퍼 함수를 잘 만들어서 쓰는게 낫다.

그리고, TS의 변환 도구를 적극적으로 사용하는 것이 좋다.

그리고 맥락이 서로 다른 타입을 억지로 통일시키지 말자. 분리되어야 하는 애들이 태그된 유니온으로 통합되면 로직을 다루기가 더 어려워질 것이다.

## 아이템 40 부정확한 타입보다는 미완성 타입 사용하기

미완성 타입이 뭘까 any타입?

타입이 구체적일수록 버그는 잘 잡히지만, 너무 구체적인건 오히려 타입이 없는 것보다 못할 수 있다.

타입을 극도로 정밀하게 만들었을 때 모든 케이스를 커버할 수 없다면 그건 오히려 잘못된 타입이다.
차라리 조금 완성이 덜 된 타입을 사용하는 것이 낫다.(모든 케이스를 커버하지는 못하는 타입)

만약 최대한 정밀하게 했는데 새로운 case가 생긴다면 개발자들은 as any를 남발할 것이다.

또한 타입스크립트의 오류 메시지를 잘 살펴보는 것도 타입 작성에 도움이 된다.

## 아이템 41 해당 분야의 용어로 타입명 짓기

매개변수명, 튜플의 라벨, 인덱스 타입의 라벨 등 이름을 지을 때 전문용어를 잘 사용하자

> 이름을 지을 때는 계산방식이 아니라 데이터 자체가 무엇인지를 고려해야함!!

잘 지은 이름은 추상화의 수준을 높이고, 의도치 않은 충돌의 위험성을 줄여준다.

## 아이템 42 데이터 형태를 추측해서 타입 만들지 않기

우리가 다루는 타입중 몇개는 프로젝트 외부에서 온 것들이다 (API,DB, 파일 등)
따라서 그 데이터의 원본 출처에서 값들이 어떻게 정의되어 있는지 확인하고 그걸 타입으로 모델링 해야한다.

음 좋은 예시로 라이브러리같은거 다운받을 때 걔네들의 type도 같이 다운받았던 적이 많은데 이번 아이템에선 이걸 말하는 것 같다.
사실 이번 아이템에서 그냥 기억에 남는건 `API Request` 타입 어떻게 정의했는지 보는정도 ^^..
