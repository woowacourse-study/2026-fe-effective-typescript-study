...
## TS스터디 27일차: 아이템 58 (6장)

### 아이템 58: 복잡한 타입 대신 코드 생성 사용하기

TS의 타입 시스템은 튜링 완전하다.

이론적으로 어떤 계산식이든 표현할 수 있음

타입 레벨에서도 조건문, 재귀, 문자열 가공 등이 가능하다는 뜻

복잡한 타입을 다루는 경우, 책에서는 DB 접근을 예시로 들었는데 타입 레벨 작업보다 코드 생성을 고려해야 한다.

외부 데이터의 경우에는 TS가 데이터의 구조를 정확히 알지 못한다.

그걸 타입으로 모델링하려고 하면 매우 복잡해지는 경우가 있다.

이럴 때 코드 생성으로 해결할 수 있다.

쉽게 설명하면 스키마를 읽고 타입을 생성해준다고 이해하면 편할 듯싶다.
